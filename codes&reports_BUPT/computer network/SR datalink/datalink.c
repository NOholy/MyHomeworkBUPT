...
#define NR_BUFS 16 //发送窗口、接收窗口大小，NR_BUFS=(MAX_SEQ+1)/2
#define inc(k) if(k < MAX_SEQ)k++;else k=0 //计算k+1  

typedef  unsigned char seq_nr;
struct FRAME { //帧的内容
	seq_nr kind; //帧的种类，分为Ack, Nak 和 Data 三种
	seq_nr ack; ////捎带确认ack
	seq_nr seq; //本帧的序号
	seq_nr info[PKT_LEN]; //数据,存储大小256
};
...
static unsigned char too_far = NR_BUFS; //接收方窗口的下界
static int phy_ready = 0; //物理层是否ready
bool arrived[NR_BUFS]; //接收方输入缓存的窗口， 
...
static void add_crc32(unsigned char *frame, int len){ 
	//将frame做CRC校验后附检错码一起发送到物理层
	*(unsigned int *)(frame + len) = crc32(frame, len);
	send_frame(frame, len + 4);// 发送到物理层，4字节检错 
	phy_ready = 0; //将数据发送到物理层后，将物理层置为忙碌状态
}

static void send_data_frame(unsigned char frame_nr){//发送数据帧 
	//将输出缓存打包成frame后调用add_crc32发送到物理层，同时开始计时data_timer
	struct FRAME s;

	s.kind = FRAME_DATA;
	s.seq = frame_nr;
	s.ack = (frame_expected + MAX_SEQ) % (MAX_SEQ + 1);
	memcpy(s.info, out_buffer[frame_nr % NR_BUFS], PKT_LEN); //将out_buffer里的缓存复制到帧frame的info域中
	
	dbg_frame("Send DATA %d %d, ID %d\n", s.seq, s.ack, *(short *)s.info);
	
	add_crc32((unsigned char *)&s, 3 + PKT_LEN); //将打包好的帧发送到物理层，3字节控制信息 
...
static void send_nak_frame(void){
	...
	add_crc32((unsigned char *)&s, 2);//将打包好的帧发送到物理层
	stop_ack_timer();
}
...
int main(int argc, char **argv)
{
	...
	int i;//arrived[]和into buffer[]的序号； 
	for (i = 0; i < NR_BUFS; i++) //接收窗口初始化 
		arrived[i] = false;
	int event, oldest_frame;//事件和最初识没有确认的帧序号 
	struct FRAME f;
	int len = 0; //收到data的长度
	...
	for (;;)
	{
			...
			case FRAME_RECEIVED: //当物理层收到一帧时
				...
				if (f.kind == FRAME_DATA) //如果是数据帧
				{
					...
					//如果收到的帧在接收方窗口内且这一帧未被接收过
					{
						...
						memcpy(in_buffer[f.seq % NR_BUFS], f.info,PKT_LEN); //将接收到的帧的data域复制至输入缓存中
						while (arrived[frame_expected % NR_BUFS])//当收到接收方窗口下界的一帧时，对这一帧以及之后收到的帧进行处理
						{
							...
							inc(frame_expected); //接收方窗口下界模（MAX_SEQ+1)加1
							inc(too_far);//接收方窗口上界模（MAX_SEQ+1)加1 
		...
		if (nbuffered < NR_BUFS && phy_ready) //如果物理层ready且目前缓存未满，则使网络层ready
